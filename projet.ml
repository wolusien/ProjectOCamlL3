...
draw_rect 100 10 100 30;;
draw_rect 250 10 100 30;;
draw_rect 450 10 100 30;;
set_color red;;
moveto 130 20;;
draw_string "START";;
moveto 290 20;;
...
let rec next()=
  ...
  let x=e.mouse_x
  and  y=e.mouse_y in
  if (x>250)&&(x<350)&&(y>10)&&(y<40) then
begin
    inversion();(fill_grid tabRef dim)
end;
  if  (x>100)&&(x<200)&&(y>10)&&(y<40) then
begin
    inversion1();(fill_grid tabRef dim);a:=(next_generation tabRef auto)
end;
 (*if  (x>450)&&(x<550)&&(y>10)&&(y<40) then
begin
    a:=(create_gen (tradsol (List.nth (readf (open_in "soluce.txt")) 1)) dim);inversion();(fill_grid tabRef dim)
end;*)
next();;
